<div class="page products">
	<section class="content-wrapper w_grid w_4col w_gap_medium">
		<section class="sidebar w_hide_view_medium w_1scol" data-url="/ext/Products/Request.php"
			data-search="{$smarty.get.text|escape:'url'}">
			{if $childsNav}
			...
			{/if}
			{foreach name="out" item="item" from=$filtersNav}
				{assign var="key" value=$item.0.PId}
				{assign var="hide" value=""}
				<div class="nav-filters nav-filters-{$key}" data-id='{$key}'>
					<div class="title">{$item.0.PName}</div>
					<ul>
						...
						<label class="w_label w_select w_select">
							<select data-minimum-results-for-search="Infinity">
								{foreach name="out" key="key" item="item" from=$productsSorting}
									<option value="{$key}" {if $productsSortingActive==$key} selected="selected" {/if}>
										{$item}</option>
								{/foreach}
							</select>
						</label>
