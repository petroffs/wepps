	...
	<form class="list-data" action="javascript:formSenderWepps.send('save','list-data-form','/packages/WeppsAdmin/Lists/Request.php')"
		...
		<div class="item pps_flex pps_flex_row pps_flex_start pps_flex_row_str pps_flex_margin" data-id="{$key}" data-group="{$item.0.FGroup}">
			...
			<div class="field pps_flex_23">
				...
				{if $element.$key}
				<div class="files pps_animate_none">
					{foreach name="o" item="i" key="k" from=$element.$key}
					<div class="files-item pps_flex pps_flex_row pps_flex_row_str pps_flex_margin" data-title="{$i.Name}" data-id="{$i.Id}">
						<div class="pps_flex_13">
						{if $i.Name|@stristr:"jpg" || $i.Name|@stristr:"jpeg" || $i.Name|@stristr:"png"}
						<a href="/f{$i.FileUrl}"><img src="/pic/lists{$i.FileUrl}" class="pps_shadow"/></a>
						{else}
						<a href="/f{$i.FileUrl}">{$i.Name}</a>
						{/if}
						</div>
