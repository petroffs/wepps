	...
	<form class="list-data"
		...
		{if $tabs|@count>1 || $listSettings.ActionShowIdAddons}
			<div class="controls-area controls-tabs">
				...
				{foreach name="out" item="item" key="key" from=$tabs}
					<a href="" class="pps_button" data-id="{$key}"><i class="fa fa-caret-right"></i>
		...
		{foreach name="out" item="item" key="key" from=$listScheme}
			{if $item.0.$listMode!="hidden"}
				<div class="item pps_flex pps_flex_row pps_flex_start pps_flex_row_str" data-id="{$key}"
					...
					<div class="field pps_flex_23">
						...
						{elseif $item.0.Type=="file"}
							{if $element.$key.0}
								<div
									class="files pps_animate_none pps_flex pps_flex_row pps_flex_row_str pps_flex_start pps_flex_margin_small">
									{foreach name="o" item="i" key="k" from=$element.$key}
										<div class="files-item files-item-2 pps_flex_16 pps_flex_13_view_medium pps_flex_12_view_medium"
									...
									{foreach name="o1" item="i1" from=$element[$key|cat:"_Rows"]}
										<div class="minitable-body minitable-row pps_flex pps_flex_row pps_flex_row_str pps_flex_start">
											{foreach name="o" key="k" item="i" from=$element[$key|cat:"_Headers"]}
												<div class="minitable-cell pps_flex_16 pps_flex_14_view_small"
											...
											{if $i.PType=='select'}
												...
												{if $item.0.$listMode!="disabled"}
													<div class="pps_flex pps_flex_row pps_flex_end">
														...
														<div class="pps_flex_fix"><a href="" class="pps_button properties-item-option-add"><i
																	class="fa fa-plus"></i></a></div>
													</div>
												{/if}
											{elseif $i.PType=='text-multi'}
												<label class="pps pps_area">
													<textarea name="pps_property_{$key}_{$i.Id}" {if $item.0.$listMode=="disabled"}
															disabled="disabled"
