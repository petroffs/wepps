...
- `LanguageId` - ID языка
- `IsHidden` - флаг видимости (0=виден, 1=скрыт)
...
#### Отладка через Utils::debug()
Метод `Utils::debug($data, $mode, $filePath)` для вывода отладочной информации. **Во время разработки** используется только в **окончательном состоянии** кода (например, для логирования ошибок).

**Параметры:**
- `$data` - переменная для отладки (любой тип: массив, строка, объект и т.д.)
- `$mode` - режим вывода (второй параметр, **ВСЕГДА ЧИСЛО**, не строка):
  - `0` - HTML-блок в браузер без закрытия соединения (по умолчанию)
  - `1` - HTML-блок + закрытие соединения
  - `2` - Перезаписать файл debug.conf (очистить и написать заново)
  - `3` - Сырой текст на экран
  - `21` - Перезаписать файл + закрытие соединения
  - `22` - Дописать в конец файла debug.conf (append)
  - `31` - Сырой текст + закрытие соединения
- `$filePath` (опционально) - путь к кастомному файлу логирования

**Примеры правильного использования:**
```php
// Вывести в браузер (только для разработки)
Utils::debug($data, 0);

// Залогировать в debug.conf
Utils::debug($data, 2);

// Залогировать и закрыть соединение (для исключений)
Utils::debug($error, 21);

// Залогировать в кастомный файл
Utils::debug($data, 2, __DIR__ . '/custom.log');
```

**Когда использовать:**
- ✅ Обработка исключений (Exception.php)
- ✅ Ошибки подключения к БД (Connect.php)
- ✅ Временная отладка во время разработки (потом удалить)
- ❌ НЕ оставлять в production-коде без причины

### Правила для Git-коммитов (commit-message-generation)

**КРИТИЧЕСКИ ВАЖНО:**
- Анализировать **ТОЛЬКО изменные файлы** (то, что показано в `git diff` и/или  `git diff --staged`)
- **ЗАПРЕЩЕНО** упоминать файлы, которых нет в git diff изменениях
- **ЗАПРЕЩЕНО** придумывать изменения — описывать только то, что реально есть в диффе
- Если изменения незначительные — писать короткий коммит, не раздувать искусственно

**Формат:**
- Заголовок: `<type>: <краткая суть>` (до 72 символов)
- Пустая строка
- Тело: список **реальных** изменений из staged diff с маркерами (-)
...
**Правила:**
1. Использовать страдательный залог (добавлен, исправлен, обновлён).
2. Избегать конструкций с "был/была".
3. Писать на русском языке.
4. **Описать ТОЛЬКО те файлы и изменения, которые реально есть в staged diff.**
5. Если файл в `.gitignore` — **НЕ УПОМИНАТЬ** его в коммите.
6. Если изменений мало — писать короткий коммит, не выдумывать дополнительные пункты.
...
- **Служебные файлы**: Служебные скрипты и описательные файлы держать в `.tools/`, не в корне проекта
