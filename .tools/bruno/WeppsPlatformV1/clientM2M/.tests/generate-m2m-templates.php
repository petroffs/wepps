 ...
 * 
 * Creates:
 * - goods/goods-get-response.json           (все товары с пагинацией)
 * - goods/goods-get-response-sample-3.json  (первые N товаров)
 * - goods/goods-post-template.json          (для CREATE всех товаров)
 * - goods/goods-post-template-sample-3.json (для CREATE первых N)
 * - goods/goods-put-template.json           (для UPDATE всех товаров с пагинацией маркером)
 * - goods/goods-put-template-sample-3.json  (для UPDATE первых N)
 */

// Определяем пути (скрипт находится в .tools/bruno/WeppsPlatformV1/clientM2M/tests/)
$rootPath = realpath(__DIR__ . '/../../../../../');
$configPath = $rootPath . '/config.php';
$outputPath = __DIR__ . '/goods';
